...
class CSmartML:
	def __init__(self, filename, population, time_budget, meta_cvi=False, algorithm=None, cvi=None, dataset=None, result="multi"):
		...
		if meta_cvi:
			if algorithm:
				...
				self.algorithm = Algorithm(filename, "distance", self.data).search()
			# self.algorithm = "birch"
			print({
				"label": "Getting recommended metrics...",
				"algorithm": self.resolve_abbr()})
			self.cvi = Meta_CVI(filename, "distance", self.data).search(self.algorithm)
			# self.cvi = [['i_index', 1], ['modified_hubert_t', 1], ['banfeld_raferty', -1]]
			print({
				"label": "Generating hyper-partitions...",
				"metric": self.resolve_metrics()})
		else:
			self.algorithm = algorithm
			self.cvi = cvi
		...
		print({"label": "{} hyper-partitions generated...".format(len(hp_set))})
		...
		print({
			...
			"partitions": self.resolve_partitions()})
	...
	def search(self):
		print({"label": "Initializing search, creating processes..."})
		func_dict = dict()
		...
		results = self.single_search(func_dict)
	...
	def single_search(self, partitions):
		res = dict()
		process = []
		for partition in partitions:
			...
			if func_name+'_s' == 'random_search_s':
				result = random_search_s(partitions[partition][0], partitions[partition][1], partitions[partition][2], partitions[partition][3], partitions[partition][4])
			else:
				result = ea_custom_s(partitions[partition][0], partitions[partition][1], partitions[partition][2], partitions[partition][3], partitions[partition][4], partitions[partition][5])
			res[partition] = result[partition_name]
		return res
...
def random_search_s(pid, t, s, population, toolbox):
	...
	while time.time() - start_time <= t:
		if ngen > 0:
			print({"label": "Random Search [Process-{}]: NGEN ({})".format(pid, ngen)})
		...
		print(trial_log)
		print({"partition_live": [label, trial_log]})
		# Replace the current population by the offspring
		population[:] = offspring

		ngen += 1
	return {pid: population}
...
def ea_custom_s(pid, t, population, toolbox, cxpb, mutpb):
	...
	while time.time() - start_time <= t:
		if ngen > 0:
			print({"label": "Evolutionary Algorithm [Process-{}]: NGEN ({})".format(pid, ngen)})
